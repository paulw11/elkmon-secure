...
class Elk extends EventEmitter {
  ...
  connect() {
    if (this.options.secure) {
      ...
      this.connection = tslConnect(
        ...
        this.host,
        {
          rejectUnauthorized: false, // Elk M1XEP uses self-signed certs
          minVersion: 'TLSv1.2',
          maxVersion: 'TLSv1.2'
        },
        () => this.onTLSConnect());
  ...
  onTLSConnect() {
    // If no username/password provided, then assume that auth is not required in the M1XEP settings.
    // Otherwise, the `connected` event will be emitted after successful auth in onDataReceived.
